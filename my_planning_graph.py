...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect_a in node_a1.action.effect_add:
            if effect_a in node_a2.action.effect_rem:
                return True
        for effect_r in node_a1.action.effect_rem:
            if effect_r in node_a2.action.effect_add:
                return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pre_p in node_a1.action.precond_pos:
            if pre_p in node_a2.action.effect_rem:
                return True
        for pre_n in node_a1.action.precond_neg:
            if pre_n in node_a2.action.effect_add:
                return True
        for pre_p in node_a2.action.precond_pos:
            if pre_p in node_a1.action.effect_rem:
                return True
        for pre_n in node_a2.action.precond_neg:
            if pre_n in node_a1.action.effect_add:
                return True
        return False
